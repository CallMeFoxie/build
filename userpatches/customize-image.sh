...
# YouTubeLive firmwareupdater filament_sensor_ng_orangepi thespaghettidetective_beta
OCTOPRINT_PLUGINS="navbartemp requestspinner stats displayz detailedprogress fullscreen ledstripcontrol BLTouch simpleemergencystop filamentmanager octoprint-eta webcamtab opitemp layerdisplay bedlevelingwizard rgb_status marlin_flasher Change_Filament"
...
OctoPrint() {
  groupadd octoprint -g 999
	useradd octoprint -m -u 999 -g octoprint
	apt update
	apt install -y --no-install-recommends python-pip python-setuptools psmisc wget python-wheel python-dev gcc make cmake libjpeg62-turbo-dev ffmpeg jq
	wget https://github.com/foosel/OctoPrint/archive/${OCTOPRINT_VERSION}.tar.gz -O /dev/stdout | tar xvpz -C /opt/
  wget https://downloads.arduino.cc/arduino-cli/arduino-cli-latest-linuxarm32.tar.bz2 -O /dev/stdout | tar xvpj -C /opt/
	ln -s /opt/OctoPrint-${OCTOPRINT_VERSION} /opt/octoprint
	(cd /opt/octoprint && pip install -e .)
  pip install numpy
	cp /tmp/overlay/octoprint.service /lib/systemd/system/octoprint.service
	ln -s /lib/systemd/system/octoprint.service /etc/systemd/system/multi-user.target.wants/
  ...
  chown -R ashley:ashley /home/ashley/.ssh/
...
InstallOctoprintPlugin() {
  ...
  if [ ! -f /tmp/plugins.json ]; then
    echo "Pre-downloading plugins list to cache"
    wget https://plugins.octoprint.org/plugins.json -O /tmp/plugins.json -q
  fi
  echo "Installing octoprint module: ${plugin}"
  url=$(cat /tmp/plugins.json | jq ".[] | select(.id == \"${plugin}\") | .archive" -r)
  #sudo -u octoprint pip install ${url} --user
  pip install ${url} # investigate if we can do plugins system-wide or not
}
